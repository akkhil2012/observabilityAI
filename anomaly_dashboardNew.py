#Please modify below file so that before click on trigger analysis I get an option to upload the .csv file import streamlit as st
import pandas as pd
import numpy as np
    ...
    with col2:
        if st.session_state.current_node is not None:
            ...
            if st.button("🚀 Trigger Analysis", key=f"trigger_{node_id}"):
                # Create a sample DataFrame with node data
                # Replace this with your actual data collection logic
                data = {
                    'timestamp': [pd.Timestamp.now()] * 3,
                    'node_id': [node_id] * 3,
                    'value': [10.5, 12.3, 11.8]  # Example feature values
                }
                ...
                params = {
                    'selected_features': ['value'],  # Replace with your actual feature columns
                    'contamination': 0.1,
                    'random_state': 42
                }
